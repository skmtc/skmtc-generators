...
export type UnionHint = {
  /** The sealed interface's name — the refName for top-level hints,
...
type HintMaps = {
   ...
   * The rendering site (`KtUnion`) throws the reason — loud per-item
...
const toHintMaps = (context: GenerateContextType): HintMaps => {
  ...
  for (const [refName, perRef] of Object.entries(namespace)) {
    ...
    // Top-level union hint: the refName IS the sealed interface.
      ...
      if (!name) {
        maps.invalid.set(
          ...
          `union hint on '${refName}.properties.${propName}' is missing 'name' — an inline ` +
            `union needs an enrichment-supplied sealed interface name`
        )
        continue
      }
 ...
 * membership scan; read by `KtUnion` to fail the item loudly). */
