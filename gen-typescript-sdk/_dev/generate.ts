// Dev harness: run gen-typescript-sdk over the `models` slice and write the
// artifacts to _dev/out/, for a Prettier-normalised diff against
// openai-node/src/resources/models.ts.
//
...
const openApiDocument = {
  ...
  components: {
    ...
    schemas: {
      ...
      DeleteModelResponse: {
        type: 'object',
        properties: { id: { type: 'string' }, deleted: { type: 'boolean' }, object: { type: 'string' } },
...
const modelsFile = '@/resources/models.ts'
...
const enrichments = {
  ...
  // Model renames + co-location live on gen-typescript-s, per ref.
  '@skmtc/gen-typescript-s': {
    Model: { main: { exportPath: modelsFile } },
    DeleteModelResponse: { main: { name: 'ModelDeleted', exportPath: modelsFile } }
  }
}
...
if (manifest.parseIssues?.length) {
  console.log('parseIssues:', JSON.stringify(manifest.parseIssues.map(i => i.type)))
}
if (!Object.keys(artifacts).length) {
  console.log('NO ARTIFACTS — results:', JSON.stringify(manifest.results ?? {}, null, 2))
}
