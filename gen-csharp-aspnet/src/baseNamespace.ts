...
export const setBaseNamespace = (value: string): void => {
  for (const segment of value.split('.')) {
    if (!isCsIdentifierName(segment) || csHardKeywords.has(segment)) {
      throw new Error(
        `@skmtc/gen-csharp-aspnet: baseNamespace '${value}' is not a valid C# namespace name — ` +
...
export const getBaseNamespace = (): string => {
  if (baseNamespace === undefined) {
    throw new Error(
      `@skmtc/gen-csharp-aspnet: baseNamespace is not set — construct the entry with ` +
        `toCsharpAspnetEntry({ baseNamespace: 'Acme.Api' })`
    )
  }
